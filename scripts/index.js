...
body.onload = function () {
  fetch(`https://api.cryptonator.com/api/full/btc-eur`)
    ...
    .then((data) => {
      document.getElementById("btc").innerHTML = data.ticker.price
        .slice(0, 12)
        .toString();
    });

  fetch(`https://api.cryptonator.com/api/full/xrp-eur`)
    .then((response) => {
      return response.json();
    })
    .then((data) => {
      document.getElementById("ripple").innerHTML = data.ticker.price
        .slice(0, 8)
        .toString();
    });

  fetch(`https://api.cryptonator.com/api/full/eos-eur`)
    .then((response) => {
      return response.json();
    })
    .then((data) => {
      document.getElementById("eos").innerHTML = data.ticker.price
        .slice(0, 8)
        .toString();
    });

  fetch(`https://api.cryptonator.com/api/full/ltc-eur`)
    .then((response) => {
      return response.json();
    })
    .then((data) => {
      document.getElementById("ltc").innerHTML = data.ticker.price
        .slice(0, 8)
        .toString();
    });

  fetch(`https://api.cryptonator.com/api/full/eth-eur`)
    .then((response) => {
      return response.json();
    })
    .then((data) => {
      document.getElementById("eth").innerHTML = data.ticker.price
        .slice(0, 8)
        .toString();
    });
};
...
form.addEventListener("click", function (e) {
  ...
  fetch(
    ...
    .then((data) => {
      ...
      console.log(total);
      document.getElementById("result").value = total.toString().slice(0, 9);
    })
    .catch((e) => {
      console.log(e + "error");
    });
